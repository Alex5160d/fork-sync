...
async function run() {
    ...
    const getTags = core.getBooleanInput('get_tags', { required: false });
    ...
    try {
        ...
        if (getTags) {
            let tags = await octokit.rest.repos.listTags({ owner, repo });
            let forkTags = await octokit.rest.repos.listTags({ owner: context.repo.owner, repo: context.repo.repo });
            let forkTagNames = forkTags.data.map(tag => tag.name);
            let parentTagNames = tags.data.map(tag => tag.name);

            // Delete tags in fork that no longer exist in parent
            for (const tag of forkTags.data) {
                if (!parentTagNames.includes(tag.name)) {
            ...
            // Create or update tags in fork
            for (const tag of tags.data) {
                if (!forkTagNames.includes(tag.name)) {
                    ...
                    let forkTag = forkTags.data.find(t => t.name === tag.name);
                    if (forkTag.commit.sha !== tag.commit.sha) {
            ...
            console.log('Tags from upstream repository have been synchronized with the fork repository.');
        }
    }
    catch (error) {
